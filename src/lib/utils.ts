...
/**
 * Sanitize billing header from system prompt
 * Removes lines starting with "BILLING_HEADER:" for cleaner prompts
 */
export const sanitizeBillingHeader = (text: string): string => {
  return text
    .split("\n")
    .filter((line) => !line.startsWith("BILLING_HEADER:"))
    .join("\n")
}
