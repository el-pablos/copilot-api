import consola from "consola"

import type {
  ChatCompletionsPayload,
  Message,
} from "~/services/copilot/create-chat-completions"
import type { Model } from "~/services/copilot/get-models"

import { getMaxContextTokensOverride, isTruncationDisabled } from "~/lib/config"
import { logEmitter } from "~/lib/logger"
import { findEndpointModel } from "~/lib/models"
import { state } from "~/lib/state"
import { getTokenCount } from "~/lib/tokenizer"

// Fallback token limits when model capabilities are not available
// These are conservative limits based on Copilot API's actual limits
const FALLBACK_TOKEN_LIMITS: Record<string, number> = {
  // Claude models via Copilot API have 200K max prompt tokens
  claude: 190000, // 200K - 10K output reserve
  // GPT models
  gpt: 120000, // Conservative estimate
  // Gemini models
  gemini: 90000, // Conservative due to tokenizer differences
  // Default fallback
  default: 100000,
}

function isSystemOrDeveloper(msg: Message): boolean {
  return msg.role === "system" || msg.role === "developer"
}

/**
 * Remove the oldest non-system message from the list, along with any
 * orphaned tool response messages if the removed message was an assistant
 * message with tool_calls.
 */
function removeOldestWithToolCleanup(messages: Array<Message>): Array<Message> {
  const [removed, ...rest] = messages
  if (removed.role !== "assistant" || !removed.tool_calls?.length) {
    return rest
  }

  const toolCallIds = new Set(removed.tool_calls.map((tc) => tc.id))
  const firstNonTool = rest.findIndex(
    (msg) =>
      msg.role !== "tool"
      || !msg.tool_call_id
      || !toolCallIds.has(msg.tool_call_id),
  )
  return firstNonTool === -1 ? [] : rest.slice(firstNonTool)
}

/**
 * Remove orphaned messages after truncation:
 * 1. Tool messages whose tool_call_id has no matching assistant tool_calls
 * 2. Assistant messages with tool_calls whose tool results are missing
 *
 * After cleanup, strips any leading tool/assistant-only messages so the
 * non-system portion always starts with a user message.
 */
function removeOrphanedToolMessages(messages: Array<Message>): Array<Message> {
  const assistantToolCallIds = new Set<string>()
  for (const msg of messages) {
    if (msg.role === "assistant" && msg.tool_calls?.length) {
      for (const tc of msg.tool_calls) {
        assistantToolCallIds.add(tc.id)
      }
    }
  }

  const toolResultIds = new Set<string>()
  for (const msg of messages) {
    if (msg.role === "tool" && msg.tool_call_id) {
      toolResultIds.add(msg.tool_call_id)
    }
  }

  const cleaned = messages
    .map((msg) => {
      if (msg.role === "assistant" && msg.tool_calls?.length) {
        const hasAllResults = msg.tool_calls.every((tc) =>
          toolResultIds.has(tc.id),
        )
        if (!hasAllResults) {
          if (msg.content) {
            const { tool_calls: _, ...rest } = msg
            return rest as Message
          }
          return null
        }
      }

      if (msg.role === "tool" && msg.tool_call_id) {
        return assistantToolCallIds.has(msg.tool_call_id) ? msg : null
      }

      return msg
    })
    .filter((msg): msg is Message => msg !== null)

  const firstUserIdx = cleaned.findIndex((m) => m.role === "user")
  if (firstUserIdx > 0) {
    return cleaned.slice(firstUserIdx)
  }
  return cleaned
}

async function computeInputTokens(
  payload: ChatCompletionsPayload,
  model: Model,
): Promise<number | null> {
  try {
    const count = await getTokenCount(payload, model)
    return count.input
  } catch {
    return null
  }
}

function resolvePromptReserve(
  tokenLimit: number,
  maxOutputTokens: number | undefined,
): number {
  const baselineReserve = Math.max(4096, Math.floor(tokenLimit * 0.05))
  if (maxOutputTokens && maxOutputTokens > 0) {
    return Math.min(maxOutputTokens, baselineReserve)
  }
  return baselineReserve
}

/**
 * Get safety multiplier for models with tokenizer mismatch.
 * Non-OpenAI models (Gemini, etc.) use different tokenizers, so our
 * GPT-based token counting may underestimate actual token usage.
 * Apply a safety margin to avoid exceeding the real limit.
 */
function getTokenizerSafetyMultiplier(modelId: string): number {
  // Gemini models: their tokenizer typically produces ~40-70% more tokens
  // than GPT tokenizers for the same content
  if (modelId.startsWith("gemini")) {
    return 0.5 // Use only 50% of calculated limit for safety
  }
  // Claude/Anthropic models: GitHub Copilot provides accurate max_prompt_tokens
  // from their server, so we trust the limit without heavy safety margin.
  // A small 5% margin is sufficient for edge cases.
  if (modelId.startsWith("claude")) {
    return 0.95
  }
  // Other non-OpenAI models may also have tokenizer differences
  if (
    !modelId.startsWith("gpt-")
    && !modelId.startsWith("o1")
    && !modelId.startsWith("o3")
    && !modelId.startsWith("o4")
  ) {
    return 0.7 // Use 70% for other non-OpenAI models
  }
  return 1.0 // OpenAI models use accurate tokenizer
}

export function resolvePromptTokenLimit(
  limits:
    | {
        max_prompt_tokens?: number
        max_context_window_tokens?: number
        max_output_tokens?: number
      }
    | undefined,
  modelId?: string,
): number | null {
  const safetyMultiplier = modelId ? getTokenizerSafetyMultiplier(modelId) : 1.0

  if (limits?.max_prompt_tokens) {
    const promptLimit = limits.max_prompt_tokens
    const outputReserve = resolvePromptReserve(
      promptLimit,
      limits.max_output_tokens,
    )
    const effectiveLimit = Math.max(promptLimit - outputReserve, 1024)
    return Math.floor(effectiveLimit * safetyMultiplier)
  }

  if (limits?.max_context_window_tokens) {
    const contextWindow = limits.max_context_window_tokens
    const outputReserve =
      limits.max_output_tokens ?
        Math.min(limits.max_output_tokens, Math.floor(contextWindow * 0.1))
      : Math.max(4096, Math.floor(contextWindow * 0.1))
    const effectiveLimit = contextWindow - outputReserve
    return Math.floor(effectiveLimit * safetyMultiplier)
  }

  return null
}

/**
 * Get fallback token limit based on model family prefix.
 * Used when model capabilities don't include explicit limits.
 */
function getFallbackTokenLimit(modelId: string): number {
  const lowerModel = modelId.toLowerCase()

  // Check known prefixes
  for (const [prefix, limit] of Object.entries(FALLBACK_TOKEN_LIMITS)) {
    if (prefix !== "default" && lowerModel.startsWith(prefix)) {
      return limit
    }
  }

  return FALLBACK_TOKEN_LIMITS.default
}

function countTrailingToolTurnMessages(messages: Array<Message>): number {
  let count = 0
  for (let i = messages.length - 1; i >= 0; i--) {
    const msg = messages[i]
    if (msg.role === "tool") {
      count++
    } else if (msg.role === "assistant" && msg.tool_calls?.length) {
      count++
      break
    } else {
      break
    }
  }
  return count
}

/**
 * Estimate token count from messages using character-based heuristic.
 * Conservative estimate: 3 characters per token.
 */
function estimateMessageTokens(messages: Array<Message>): number {
  const totalChars = messages.reduce((sum, msg) => {
    const content =
      typeof msg.content === "string" ?
        msg.content
      : JSON.stringify(msg.content)
    return sum + content.length
  }, 0)
  return Math.ceil(totalChars / 3)
}

/**
 * Resolve max prompt tokens from model capabilities or fallback.
 */
function resolveMaxTokens(
  selectedModel: Model | undefined,
  modelId: string,
): number {
  const contextOverride = getMaxContextTokensOverride()
  if (contextOverride > 0) {
    consola.debug(`Using context tokens override: ${contextOverride}`)
    return contextOverride
  }

  if (selectedModel) {
    const limit = resolvePromptTokenLimit(
      selectedModel.capabilities.limits,
      modelId,
    )
    if (limit) return limit
  }

  const fallback = getFallbackTokenLimit(modelId)
  consola.debug(
    `[truncate] Using fallback token limit for ${modelId}: ${fallback}`,
  )
  return fallback
}

interface TruncationContext {
  payload: ChatCompletionsPayload
  selectedModel: Model | undefined
  maxPromptTokens: number
  useEstimationMode: boolean
  systemMessages: Array<Message>
  nonSystemMessages: Array<Message>
  initialInput: number
}

/**
 * Execute the truncation loop, removing oldest messages until under limit.
 */
async function executeTruncationLoop(ctx: TruncationContext): Promise<{
  messages: Array<Message>
  currentInput: number
  iterations: number
}> {
  const {
    payload,
    selectedModel,
    maxPromptTokens,
    useEstimationMode,
    systemMessages,
    nonSystemMessages,
    initialInput,
  } = ctx
  const trailingProtected = countTrailingToolTurnMessages(nonSystemMessages)
  const minKeep = Math.max(2, trailingProtected)

  let currentMessages = nonSystemMessages
  let currentInput = initialInput
  let iterations = 0
  const maxIterations = 50

  while (
    currentInput > maxPromptTokens
    && currentMessages.length > minKeep
    && iterations < maxIterations
  ) {
    iterations++
    currentMessages = removeOldestWithToolCleanup(currentMessages)

    if (useEstimationMode) {
      currentInput = estimateMessageTokens(currentMessages)
    } else if (selectedModel) {
      const truncatedPayload = {
        ...payload,
        messages: [...systemMessages, ...currentMessages],
      }
      const newInput = await computeInputTokens(truncatedPayload, selectedModel)
      if (newInput === null) break
      currentInput = newInput
    }
  }

  if (iterations >= maxIterations) {
    consola.warn(
      `Truncation reached max iterations (${maxIterations}), stopping early`,
    )
  }

  return { messages: currentMessages, currentInput, iterations }
}

export async function truncateMessages(
  payload: ChatCompletionsPayload,
): Promise<ChatCompletionsPayload> {
  if (isTruncationDisabled()) {
    consola.debug("Truncation disabled via config")
    return payload
  }

  // Try exact match first, then use findEndpointModel for normalized lookup
  const selectedModel =
    state.models?.data.find((m) => m.id === payload.model)
    ?? findEndpointModel(payload.model)

  const maxPromptTokens = resolveMaxTokens(selectedModel, payload.model)
  const estimatedTokens = estimateMessageTokens(payload.messages)

  // If estimated tokens are well below limit, skip expensive tokenization
  if (estimatedTokens < maxPromptTokens * 0.8) {
    consola.debug(
      `[truncate] Skip tokenization: estimated ${estimatedTokens} < ${Math.floor(maxPromptTokens * 0.8)} (80% limit)`,
    )
    return payload
  }

  consola.debug(
    `[truncate] Estimated ${estimatedTokens} tokens, limit ${maxPromptTokens} - running tokenization...`,
  )

  // Use estimation mode when model is not found or payload is very large
  const useEstimationMode =
    !selectedModel || estimatedTokens > maxPromptTokens * 2
  if (useEstimationMode) {
    consola.debug(
      `[truncate] Using fast estimation mode (${selectedModel ? "large payload" : "no model found"})`,
    )
  }

  // Compute initial input tokens
  const initialInput =
    useEstimationMode ? estimatedTokens : (
      ((await computeInputTokens(payload, selectedModel)) ?? estimatedTokens)
    )
  if (initialInput <= maxPromptTokens) return payload

  const systemMessages = payload.messages.filter((m) => isSystemOrDeveloper(m))
  const nonSystemMessages = payload.messages.filter(
    (m) => !isSystemOrDeveloper(m),
  )
  const originalCount = nonSystemMessages.length

  const result = await executeTruncationLoop({
    payload,
    selectedModel,
    maxPromptTokens,
    useEstimationMode,
    systemMessages,
    nonSystemMessages,
    initialInput,
  })

  const removedCount = originalCount - result.messages.length
  if (removedCount > 0) {
    let finalMessages = removeOrphanedToolMessages(result.messages)

    if (finalMessages.length === 0) {
      const lastUser = payload.messages.findLast((m) => m.role === "user")
      if (lastUser) finalMessages = [lastUser]
    }

    consola.warn(
      `Truncated ${removedCount} messages to fit within ${maxPromptTokens} prompt token limit (${initialInput} → ${result.currentInput} tokens)`,
    )
    logEmitter.log(
      "warn",
      `Truncated ${removedCount} messages: ${initialInput} → ${result.currentInput} tokens (limit: ${maxPromptTokens})`,
    )
    return { ...payload, messages: [...systemMessages, ...finalMessages] }
  }

  return payload
}
