...
async function handleWithMessagesApi(
  ...
  if (hasAdaptiveThinking && !disableThink) {
    ...
    const effortBudgetMultiplier: Record<string, number> = {
      ...
      medium: 0.5,
      high: 0.75,
      max: 1.0, // xhigh maps to max
    }
    const multiplier = effortBudgetMultiplier[effort] ?? 0.75

    // Use max budget from capabilities, or high default (128K for extended thinking)
    const maxBudget =
      selectedModel?.capabilities.supports?.max_thinking_budget ?? 128000
    const minBudget =
