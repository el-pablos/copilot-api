...
async function sendRequestWithRetry(params: {
  ...
  rebuildHeaders: (token: string) => Record<string, string>;
}): Promise<Response> {
  const { model, sendRequest, requestPayload, getToken, rebuildHeaders } =
    params;
  ...
  for (
    ...
    try {
      ...
      if (response.status === 429) {
        ...
        // Account-level rate limit - try rotating and retry
        if (
          isPoolEnabledSync() &&
          ...
          if (newToken) {
            ...
            if (newAccount?.id !== currentAccount?.id) {
              ...
              const newHeaders = rebuildHeaders(newToken);
              ...
              // If rotation succeeded, return the response
              if (retryResponse.ok || retryResponse.status !== 429) {
                return retryResponse;
              }

              // If still 429, continue to next rotation attempt
              consola.warn(
                `Still rate-limited after rotation. Continuing retries...`,
              );
              continue;
            }
          }
          ...
          consola.warn(
            `No available account after rotation. Continuing with standard retry...`,
          );
        }
      }
...
export const createChatCompletions = async (
  ...
  const response = await sendRequestWithRetry({
    ...
    getToken: getActiveCopilotToken,
    rebuildHeaders: (newToken: string) =>
      buildHeaders(normalizedPayload, newToken),
  });
  ...
  if (!response.ok) {
    ...
    if (fallbackSelection) {
      ...
      return parseSuccessfulCompletion(
        fallbackResponse,
        fallbackPayload.stream,
      );
    }
