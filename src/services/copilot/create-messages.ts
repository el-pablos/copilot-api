...
const buildAnthropicBetaHeader = (
  ...
  if (anthropicBetaHeader) {
    ...
    const finalFilteredBetas = isAdaptiveThinking
...
export const createMessages = async (
  ...
  // DEBUG: Track where request gets stuck (use warn to ensure visibility)
  consola.warn("[createMessages] 1. Starting request...");
  ...
  for (let attempt = 1; attempt <= MAX_RETRY_ATTEMPTS; attempt++) {
    try {
      ...
      consola.warn("[createMessages] 2. Getting copilot token...");
      const copilotToken = await getActiveCopilotToken();
      consola.warn("[createMessages] 3. Got copilot token");
      ...
      consola.warn(
        `[createMessages] 4. Sending to ${requestUrl} (timeout: ${config.requestTimeoutMs}ms)`,
      );
      ...
      consola.warn(
        `[createMessages] 5. Response: ${response.status} (${Date.now() - fetchStartTime}ms)`,
      );
      ...
      if (response.status === 429) {
        if (
          ...
          const newToken = await getActiveCopilotToken();
